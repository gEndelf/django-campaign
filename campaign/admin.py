...
class CampaignAdmin(admin.ModelAdmin):
    ...
    def send_view(self, request, object_id, extra_context=None):
        ...
        if request.method == 'POST':
            ...
            messages.success(
                request, _(
                    u'The %(name)s "%(obj)s" was successfully sent. '
                    u'%(num_sent)s messages delivered.' % {
                        'name': force_unicode(opts.verbose_name),
                        'obj': force_unicode(obj), 'num_sent': num_sent, }))
        ...
        def form_media():
            ...
            return forms.Media(css={
                'screen':
                    ['%sadmin/%s' % (settings.STATIC_URL, url) for url in
                     css]})
    ...
    def get_urls(self):
        ...
        urlpatterns = patterns(
            '',
            url(r'^(.+)/send/$', wrap(self.send_view),
